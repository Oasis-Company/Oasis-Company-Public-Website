...
async function onRequest(context) {
  const { env } = context;
  const info = {
    hasKV: !!env.APPLICATIONS_KV,
    kvType: typeof env.APPLICATIONS_KV,
    kvKeys: [],
    error: null
  };
  if (env.APPLICATIONS_KV) {
    try {
      info.counter = await env.APPLICATIONS_KV.get("counter:apps");
      info.proposalList = await env.APPLICATIONS_KV.get("proposal:list");
      info.prop1 = await env.APPLICATIONS_KV.get("proposal:prop-1");
    } catch (e) {
      info.error = e.message;
      info.errorStack = e.stack?.substring(0, 500);
    }
  }
...
// ../.wrangler/tmp/pages-CP2X0C/functionsRoutes-0.11363256715863934.mjs
var routes = [
  {
    routePath: "/apply/api/debug",
    mountPath: "/apply/api",
    method: "",
    middlewares: [],
    modules: [onRequest]
